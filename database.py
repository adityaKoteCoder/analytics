...
def delete_internship(id):
    try:
        with db.DbContext() as connection:
            cursor = connection.cursor()
            cursor.execute("delete internship where id=?",(id,))
...
def add_student_internship(name,sem,iid):
    try:
        with db.DbContext() as connection:
            cursor = connection.cursor()
            cursor.execute("insert into internship(name,sem,iid) values(?,?,?)",(name,sem,iid))
            print(f"Student with Name:{name} and USN:{iid} added successfully!!")
    except Exception as e:
        print(str(e))
